...
jsPsych.plugins['survey-multi-choice'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
	...
	for (var j = 0; j < trial.options[i].length; j++) {
       ...
       div.setAttribute('id', option_id_name);
       div.setAttribute('class',_join(plugin_id_name, 'option'));
       display_element.querySelector(question_selector).appendChild(div);
       //div.addEventListener('click', () => {console.log('Someone clicked me!')})
        //div.innerHTML = trial.options[i][j];
//div.addEventListener('click', function (event) { console.log(event); })
	...
	var input_id = _join(plugin_id_name, 'response', i, j);
      ...
      // create radio button
        ...
        document.getElementById(input_id).style.opacity = "0";
